...
interface FormProps {
    handleClick: (email: string, pass: string, user:any) => void;
    title: string;
    questiontag: string;
    directtag: string;
...
const Form: FC<FormProps> = ({ title, handleClick, questiontag, directtag, hrefDirection, user }) => {
    ...
    return (<div className={form.form}>
        ...
        <div className={form.mainfooter}>
            <button className={form.buttonContinue} onClick={() => handleClick(email, pass, user)}>
                {title}
            </button>
            <div className={form.questionBlock}>
                <p className={form.question}> {questiontag} </p>
