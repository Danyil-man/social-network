...
export const authAPI = {
  async reg(username: string, login: string, password: string) {
    const response = await instanceApi.post<ResponseType<RegistrationType>>("/create-account", {
      username,
      login,
      password,
    });
    instanceApi.defaults.headers.common.authorization = response.config.headers?.authorization as string;
  },

  async login(login: string, password: string) {
    const response = await instanceApi.post<ResponseType<LoginType>>("/login", {
      login,
      password,
    });
    instanceApi.defaults.headers.common.authorization = response.config.headers?.authorization as string;
