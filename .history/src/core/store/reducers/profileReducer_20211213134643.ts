...
export const getProfileUser=(username: string, userProfile: GetUserType):ThunkType => async (dispatch) => {
    dispatch(actions.setIsLoading(true))
    let response = await profileAPI.getSingleProfile(usernam, userProfilee)
    dispatch(actions.setProfileData(response.data))
    debugger
    dispatch(actions.setIsLoading(false))
}
