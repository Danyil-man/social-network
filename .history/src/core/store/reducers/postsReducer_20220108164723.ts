...
const postsReducer = (state = initialState, action: ActionCreatorsType): initialStateType => {
    switch (action.type) {
        ...
        case CREATE_POST:
            return {
                ...state,
                photos_attributes: action.photos_attributes 
            }
...
export const actions = {
    ...
    createPost: (photos_attributes: Array<ImagePhotoType>, author: PostAuthor) => ({
        type: CREATE_POST,
        photos_attributes,
        author
    } as const)
...
export const createPosts = (photos_attributes: Array<ImagePhotoType>): ThunkType => async (dispatch) => {
    dispatch(actions.isLoading(true))
    debugger
    let response = await PostsAPI.createPost(photos_attributes)
    debugger
    dispatch(actions.createPost(photos_attributes, response.data))
    dispatch(actions.isLoading(false))
}
