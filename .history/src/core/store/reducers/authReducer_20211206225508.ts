...
export const registration = (username: string, login: string, password: string):ThunkType => async (dispatch) => {
    ...
    //let messages = response.data.data.success >  response.data.data.success[0] ? "OK" : "ERROR"
    let success = response.data.data.success >  response.data.data.success[0] ? "OK" : "ERROR"
    if(response.data.data.success == "Your account has been created"){
